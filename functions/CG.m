function [traceClass,num]=CG(path)
...
for i=1:length(DataSet)
    ...
    for j=1:length(tempMatData.Size)
        ...
        smpList(smpNum+j,9)={tempMatData.Pi(j)};
    end
    smpNum=smpNum+length(tempMatData.Size);
end
cMat(smpNum,8)=0;
...
for i=1:size(classSet,1)
    ...
    for j=1:jMax
        ...
        traceClass.(['class',num2str(i)]).data(j,9)=smpList(jj,9);
    end
end
...
for i=1:size(classSet,1)
        ...
        tempMu=zeros(1,2);
        tempPi=0;
        traceClass.(['class',num2str(i)]).len
...
%save 'traceAnalyse.mat' Mu Sigma Pi
 save([path '\result\GC result.mat'],'Mu', 'Sigma');
if 0
dp=find(Pi<0.1);
