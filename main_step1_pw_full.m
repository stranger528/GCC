...
for i=1:length(DS)
    ...
    %error2Set(K)=max(tempPntSet)/min(ValueSetP(maxPos),ValueSetG(maxPos));
     ...
     error2Set(K)=sum(abs(ValueSetP-ValueSetG))*((1e-3)*(1e-3))/((XLim(2)-XLim(1))*(YLim(2)-YLim(1)));
        if K>2
            if(error2Set(K)==min(error2Set))&&((abs(error2Set(K)-error2Set(minError2Num))/error2Set(minError2Num))>0.05)
